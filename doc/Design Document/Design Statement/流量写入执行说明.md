...
工程版本：V1.8.0.260920。日期：2026-09-20。

## 1. 已实现的范围

上位机通过CAN_USER的0x01写0x0200～0x0205，MCU经已锁定的RS485/EX201或下行CAN_USER后端设置对应设备的数字流量，并在读回一致后通过0x06返回执行结果。六路共用同一套业务接口，地址、单位、小数位及量程各自独立。

只改变数字设定值，不切换数字/模拟来源，不操作内部阀设定、九个外部阀门或气路。读回成功不表示实际流量已到达目标。下行CAN/MCP2515和双协议探测已接入，探测期间拒绝流量写入。下面的RFSM/WSFD/RSFD步骤描述RS485路径；CAN对应过程见本文件末节。
...
下行250 kbit/s，FLOW节点1～6对应六通道。仅在整条总线锁定为CAN、本节点标识0x4D464301和参数表版本1均正确、九项元数据初始化完成后允许写入。

| 公共写状态 | RS485 | 下行CAN |
| --- | --- | --- |
| SOURCE | RFSM确认数字来源 | 0x02读0x0104，值必须为0 |
| START / ACK | 单次WSFD，等待OK/NG | 单次0x01写0x0200，等待0x06，值0为成功 |
| READBACK / VERIFY | RSFD读回尾数 | 0x02读0x0001，换算为本通道尾数后核对 |
| DONE | 保留结果等待入队 | 同左 |

`A_MFC_StartRead`、`A_MFC_ProcessTransport`、`A_MFC_GetInfo`、`A_MFC_GetFlow`按selected_link选择后端。`A_MfcCan_Start`只把编码后的帧复制到待发槽；下一轮授权仍有效才由`A_MfcCan_Process → F_MfcCan_Process → H_MfcCan_Process`推进SPI及CAN发送。

START处的最终检查与提交仍可在原临界区内执行，但SPI等待不在临界区内运行。启动失败保存在deferred_error，下一轮开中断后再恢复硬件。请求失效或超过3000ms总期限时，丢弃尚未发出的CAN帧；已经上总线的写入不能撤回。MCP2515 One-Shot和业务层都不自动重发写入，切换后端也不重放旧命令。

CAN单笔总期限200ms，错误后静默250ms；对端须在100ms内回复一次。原CAN_USER没有事务序号，必须遵守响应上界，不能保证识别任意超长迟到的相同参数回复。完整参数、错误码和示例见[MFC下行CAN_USER协议与自动识别](MFC下行CAN_USER协议与自动识别.md)。
