...
适用代码版本：**V1.6.1.260915**。文档修订：R3。更新日期：2026-09-15。
...
当前完成的下行路径为SCI2/RS485上的EX-201S自带协议。上位机为CAN0/CAN_USER。MFC侧MCP2515/CAN_USER、双协议自动识别、外部九阀执行及完整气路过程联锁尚未接入，不能将历史设计图中的规划状态视为当前已实现流程。
...
| ControlTask | ControlTask_entry | A_System_ProcessControl → A_Control_Process | 4 / 1024字节 | 出队领取写请求、转发MFC命令、映射结果并入CAN结果队列 |
...
三个任务的A_System_Process入口均先检查A_System_Initialize，首个进入者在临界区中创建七个静态队列；已成功创建的队列不重建。创建失败时下次重试，不清空已创建队列。queues_ready只表示启动配置完成，完成后保持不变；运行中的业务就绪、链路和请求状态都走队列。

| 执行位置 | 函数 | 建立的条件 |
| --- | --- | --- |
| 三个任务入口 | A_System_GetContext | 获得静态上下文入口和队列句柄；禁止读取其他任务的业务成员 |
| 三个A_System_Process入口 | A_System_Initialize → 七处直接调用xQueueCreateStatic | 初始化七个长度1队列；数据区及控制块均静态分配 |
...
| ControlTask | A_Control_Initialize | 只绑定本任务使用的五个队列，不取得CAN或MFC业务指针 |
| MfcTask → CAN队列 | A_System_ProcessMfc → xQueueOverwrite | 六路数据、link和ready一起复制到telemetry_queue |
| HostCanStack | A_System_ReceiveTelemetry → A_HostCan_SetExecutors | 根据出队消息启用MFC执行能力；外部阀执行位仍关闭 |
...
## 3. 七个FreeRTOS队列及任务本地缓冲

创建位置：A_System_Initialize → 七处直接调用xQueueCreateStatic。全部队列长度为1，均使用静态内存，入队和出队等待时间为0。
...
| 2 | A_System.c文件开头、A_System_Initialize | 每份静态状态属于哪个任务；七个xQueueCreateStatic分别存什么 |
...
V1.6.1已通过六路轮询、写链路、队列交接、三任务六种交替顺序测试和CAN协议回归；RA4M1 Debug构建通过，text=30108、data=2468、bss=8324字节。当前任务栈各1024字节，尚无实板验证。本文件R3对应可读性整理后的实现；24个自定义结构体按值嵌套最大两层，生产代码成员访问链最长两次。队列方向、容量、总期限和线上协议保持既有规则。H_MFC_CAN_SpiCallback目前仍是空回调；A_EX201_CloseFlow虽已有接口，但当前流量写链路不调用它；A_HostCan_PublishSystem接口保留供外部阀及整机控制使用，当前不代表九阀驱动已经实现。
