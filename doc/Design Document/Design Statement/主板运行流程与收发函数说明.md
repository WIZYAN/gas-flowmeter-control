...
适用代码版本：**V1.8.0.260920**。文档修订：R5。更新日期：2026-09-20。
...
上位机为CAN0/CAN_USER。下行已接入SCI2/RS485上的EX201原厂协议、SPI1/MCP2515上的本项目CAN_USER，以及运行时自动识别；CAN为250k、节点1～6。九阀电气执行及V7/V9与V8互斥已接入，完整气路过程和通信丢失自动关阀策略仍未接入。下行协议、示例及限制见[MFC下行CAN_USER协议与自动识别](MFC下行CAN_USER协议与自动识别.md)。
...
flowchart TD
    ...
    VQ --> HOST
    CQ --> MFC["MfcTask<br/>确认来源 → 单次写入 → 读回"]
    MFC --> RQ["result_queue"]
    RQ --> CTRL
...
所有任务间消息都通过FreeRTOS队列按值复制。A_HostCan_PublishChannel、TakeCommand、CommandActive和CompleteCommand现在都只在CAN任务内调用；这些函数不会自动切换任务。MFC轮询和写入共用当前锁定的后端：RS485使用A_EX201_Context，CAN使用A_MfcCan_Context；同一时刻只允许一个下行事务。两个后端均归MfcTask管理。
...
MfcTask_entry → A_System_ProcessMfc
  ...
  4. A_MFC_Process先调用A_MfcLink_Process探测/检查锁定，再推进六路轮询
...
| 共享硬件恢复失败 | A_MFC_TransportFailed、A_MfcLink_Process | 六路失效，停止旧事务后重新探测；停止失败时不直接换后端 |
| 错误后静默：RS485 50ms，CAN 250ms | A_MFC_Process、A_MFC_ProcessCommand | 静默结束才允许后续请求，具体迟到应答上界待实机确认 |
...
| [A_System.c](F:/project/gas-flowmeter-control/GS_flowmeter/App/A_System.c) | 上下文、队列、任务衔接和快照发布 |
| [A_Control.c](F:/project/gas-flowmeter-control/GS_flowmeter/App/A_Control.c) | 领取命令、命令入队、结果出队及错误码映射 |
...
V1.6.1已通过六路轮询、写链路、队列交接、三任务六种交替顺序测试和CAN协议回归；RA4M1 Debug构建通过，text=30108、data=2468、bss=8324字节。当前任务栈各1024字节，尚无实板验证。本文件R3对应可读性整理后的实现；24个自定义结构体按值嵌套最大两层，生产代码成员访问链最长两次。队列方向、容量、总期限和线上协议保持既有规则。V1.6.1时H_MFC_CAN_SpiCallback仍为空回调，V1.8.0已由H_MfcCan实现；A_EX201_CloseFlow虽已有接口，但当前流量写链路不调用它；V1.7.0进一步通过valve_state_queue和A_HostCan_PublishSystem接入九阀状态；以上V1.6.1资源占用保留作历史对比，当前资源和验证见README的V1.8.0条目。
...
## 14. V1.8.0 下行CAN和自动识别调用链

```text
A_MFC_Process
  → A_MfcLink_Process
      → A_MfcLink_Prepare：停止旧CAN及UART事务，再初始化本次后端
      → RS485：A_EX201_ReadFullScale / A_EX201_ReadDecimalPlaces
      → CAN：A_MfcCan_Start，读取0x0100标识 / 0x0108版本
      → 同节点两次正确应答后锁定selected_link
  → A_MFC_SelectRead / A_MFC_StartRead：轮转六路，选择已锁定后端
      → CAN各节点先分别检查标识、版本，再读取九项初始化参数
      → A_MfcCan_Start → F_CanUser_Encode → F_MfcCan_Send → H_MfcCan_Send
          只复制到MfcTask内部待发槽，没有SPI等待
  → 下一轮A_MFC_ProcessTransport
      → A_MfcCan_Process → F_MfcCan_Process → H_MfcCan_Process
          读取EFLG / CANINTF / RXB0 / RXB1，必要时载入TXB0并RTS
          H_MFC_CAN_SpiCallback只记录单条SPI命令完成或失败
      → F_MfcCan_Receive → F_CanUser_Decode
          核对来源、目标、功能、参数、数量和校验
      → A_MfcCan_GetInfo / A_MfcCan_GetFlow
          逐字段范围检查，float32转换为当前通道尾数
  → A_MFC_Finish：更新本通道缓存、有效位、revision、last_valid_tick
  → A_System_MapChannel → telemetry_queue → CAN任务缓存
```

六路公平调度和八个队列保持不变。第4～5节的EX201函数表适用于RS485分支；CAN参数映射见[六路MFC轮询说明](六路MFC轮询说明.md)末节。

CAN写流程为`0x02读0x0104 → 0x01写0x0200 → 0x02读0x0001`。与RS485一样，只有来源合法、目标合规、写应答成功且读回一致才向上位机返回成功。START处只入本任务待发槽；下一轮再次检查授权后才访问SPI，失败恢复也在开中断状态执行。

调试时优先看`selected_link`、`probe_link`、`probe_channel`、`probe_step`、`last_valid_tick`、每路`can_profile_checked`及`initialize_step`，再看A_MfcCan的`node/parameter/state/device_error/rejected_frames`和H_MfcCan的`fault/transmit_state/spi_error`。CAN硬件ACK不能替代CAN_USER业务回复。

单路失联保持现有后端。整链路10秒无有效响应或恢复失败后，A_MfcLink清缓存、停止旧事务并重新只读探测；失败写不在新后端重放。停止旧硬件失败时不直接切换，气路自动停机策略另行实现。本版已通过模拟和ARM编译，实板电平、总线及执行器尚未验证。
